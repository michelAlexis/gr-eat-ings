...
import {
  type ApiProduct,
  searchApiByBarcode,
} from "$lib/server/food-facts/api";
    ...
    async start(controller) {
      ...
      try {
        ...
        if (barcode) {
          ...
          const dbResults = await searchIngredientDbName(
            search.query,
            search.limit,
          );
          await wait(1000);
...
async function searchIngredientDbBarcode(
  barcode: string,
...
async function createIngredientByProduct(
  product: ApiProduct,
): Promise<IngredientSearchResult> {
  return await db.transaction(async (tr) => {
    ...
    const created = await tr
      ...
      .values({
        ...
        refQuantity: product.serving_quantity_unit
        barcode: product.id,
